...
class Player(pygame.sprite.Sprite):
    ...
    def dano_player(self):
        ...
        #usar o coliderect para ver os sprites de dano
    ...
    def importar(self):
        ...
        self.animations_dmg = {'up': ['prota_dmg_up.png'], 'down':  ['prota_dmg_down.png'], 
                           'left': ['prota_dmg_left.png'], 'right':  ['prota_dmg_right.png'], 
                           'up_idle':  ['prota_dmg_up.png'], 'down_idle':  ['prota_dmg_down.png'], 
                           'left_idle':  ['prota_dmg_left.png'], 'right_idle':  ['prota_dmg_right.png']}
    ...
    def animar(self):
        ...
        if not self.dano:
            ...
            animation = self.animations_dmg[self.status]
...
class Zumbi(pygame.sprite.Sprite):
    ...
    def __init__(self, pos, group):
        ...
        self.rect = self.image.get_rect(center=pos)
        self.direction_zumbi = pygame.math.Vector2()
        self.speed = 3.3
        self.status = 'down'
        self.frame_index = 0
        self.animation_speed = 0.15
    ...
    def zombie_input(self):
        keys = pygame.key.get_pressed()
        if keys[pygame.K_UP]:
            self.direction_zumbi.y = -1
            self.status = 'up'
        elif keys[pygame.K_DOWN]:
            self.direction_zumbi.y = 1
            self.status = 'down'
        else:
            self.direction_zumbi.y = 0
        if keys[pygame.K_LEFT]:
            self.direction_zumbi.x = -1
            self.status = 'left'
        elif keys[pygame.K_RIGHT]:
            self.direction_zumbi.x = 1
            self.status = 'right'
        else:
            self.direction_zumbi.x = 0
    def get_status(self):
        if self.direction_zumbi.x == 0 and self.direction_zumbi.y == 0:
            if not 'idle' in self.status:
                self.status = self.status + '_idle'
    ...
    def update(self):
        self.zombie_input()
        self.rect.center += self.direction_zumbi * self.speed 
        self.get_status()
        self.animar()
